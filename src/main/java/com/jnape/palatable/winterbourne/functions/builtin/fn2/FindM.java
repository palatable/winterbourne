 ...
 * {@link Maybe#nothing()} wrapped in the effect <code>M</code>. This function short-circuits, and so is safe to use
 * on potentially infinite {@link StreamT} instances that guarantee to emit a matching element * eventually.
 *
...
public final class FindM<M extends MonadRec<?, M>, A, MMA extends MonadRec<Maybe<A>, M>>
        implements Fn2<Fn1<? super A, Boolean>, StreamT<M, A>, MMA> {
    ...
    @Override
    public MMA checkedApply(Fn1<? super A, Boolean> predicate, StreamT<M, A> as) {
